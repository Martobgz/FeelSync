...
// =======================
void usendtoble(uint16_t variable) {
  // send logic goes here later
}
...
void loop() {
  ...
  if (now - lastSendTime >= SEND_INTERVAL_MS) {
    ...
    // Send the 4 final values
    usendtoble(heartRateAvg5Min);
    usendtoble(spo2Avg5Min);
    usendtoble(accelerometerState5Min);
    usendtoble(gsrState5Min);
